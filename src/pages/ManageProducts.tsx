...
function mapProductToForm(product: Product): ProductFormState {
  return {
    ...
    minOrderQty:
      product.minOrderQty !== undefined ? String(product.minOrderQty) : "",
  };
}
...
function validateProductInput(source: ProductFormState): Product {
  ...
  const gstRate = source.gstRate.trim()
    ? Number(source.gstRate.trim())
    : undefined;
  const listPrice = source.listPrice.trim()
    ? Number(source.listPrice.trim())
    : undefined;
  const discountPercent = source.discountPercent.trim()
    ? Number(source.discountPercent.trim())
    : undefined;
  const minOrderQty = source.minOrderQty.trim()
    ? Number(source.minOrderQty.trim())
    : undefined;

  if (!id || !name || !description || !category || images.length === 0) {
    throw new Error(
      "Fill all required fields and include at least one image URL.",
    );
  }
  ...
  if (
    ...
    (Number.isNaN(minOrderQty) || minOrderQty < 1)
  ) {
...
const ManageProducts = () => {
  ...
  const onDeleteProduct = async (product: Product) => {
    const shouldDelete = window.confirm(
      `Delete "${product.name}" (${product.id}) from shop products?`,
    );
    if (!shouldDelete) {
      return;
    }
  ...
  const onCsvUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(
        `Bulk upload complete. Created: ${created}, Updated: ${updated}`,
      );
    } catch (error) {
      toast.error(
      ...
      <div className="container mx-auto px-4 py-10 space-y-6">
        <header className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Products and categories arranged in tables. Use the buttons to bring
            up the forms when needed.
          </p>
        </header>
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Browse the catalog, bulk upload CSV files, reset to seed data,
                  or edit an entry via the table actions.
                </p>
              </div>
              <button
                type="button"
            ...
            {/* <div className="space-y-2 rounded-xl border border-border bg-background p-4 text-xs text-muted-foreground">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span>
                  Bulk upload CSV{" "}
                  <code>id,name,price,description,category,images</code>{" "}
                  (optional:
                  gstRate,listPrice,discountPercent,availableColors,productCode,material,packingType,masterCarton,customized,minOrderQty).
                  Use <code>|</code> to combine multiple values.
                </span>
              </div>
            ...
            {productFormOpen && (
              <div className="rounded-xl border border-border bg-card p-6 space-y-4">
                ...
                <form onSubmit={onSubmitSingle} className="space-y-4">
                  ...
                  <label className="text-sm space-y-1 block">
                    ...
                    <div className="flex flex-wrap items-center gap-3 pt-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        Upload adds a public URL to the Images field (bucket:
                        product-images).
                      </span>
                    </div>
                  </label>
                  ...
                  <label className="text-sm space-y-1 block">
                    <span className="text-muted-foreground">
                      Available Colors (optional, use |)
                    </span>
                    <input
                      value={form.availableColors}
                      onChange={(e) =>
                        onFormChange("availableColors", e.target.value)
                      }
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <label className="text-sm space-y-1">
                      ...
                      <input
                        value={form.gstRate}
                        onChange={(e) =>
                          onFormChange("gstRate", e.target.value)
                        }
                        placeholder="18"
                        className="w-full rounded-lg border border-border bg-background px-3 py-2"
                      />
                    ...
                    <label className="text-sm space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          onFormChange("listPrice", e.target.value)
                        }
                        placeholder="4847.5"
                        className="w-full rounded-lg border border-border bg-background px-3 py-2"
                      />
                    ...
                    <label className="text-sm space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          onFormChange("discountPercent", e.target.value)
                        }
                        placeholder="30"
                        className="w-full rounded-lg border border-border bg-background px-3 py-2"
                      />
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <label className="text-sm space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          onFormChange("productCode", e.target.value)
                        }
                        placeholder="H1512"
                        className="w-full rounded-lg border border-border bg-background px-3 py-2"
                      />
                    ...
                    <label className="text-sm space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          onFormChange("minOrderQty", e.target.value)
                        }
                        placeholder="7"
                        className="w-full rounded-lg border border-border bg-background px-3 py-2"
                      />
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <label className="text-sm space-y-1">
                      ...
                      <input
                        value={form.material}
                        onChange={(e) =>
                          onFormChange("material", e.target.value)
                        }
                    ...
                    <label className="text-sm space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          onFormChange("packingType", e.target.value)
                        }
                  ...
                  <label className="text-sm space-y-1 block">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onFormChange("masterCarton", e.target.value)
                      }
                  ...
                  <label className="text-sm space-y-1 block">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onFormChange("customized", e.target.value)
                      }
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Edit or add categories beneath the table.
                </p>
              </div>
              <button
                type="button"
                ...
                >
                  <label className="text-sm space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onCategoryFormChange("id", e.target.value)
                      }
                      placeholder="auto-generated-from-name"
                      className="w-full rounded-lg border border-border bg-background px-3 py-2"
                    />
                  ...
                  <label className="text-sm space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onCategoryFormChange("name", e.target.value)
                      }
                      className="w-full rounded-lg border border-border bg-background px-3 py-2"
                    />
                  </label>
                  <label className="text-sm space-y-1 md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onCategoryFormChange("description", e.target.value)
                      }
                      className="w-full rounded-lg border border-border bg-background px-3 py-2"
                    />
                  </label>
                  <label className="text-sm space-y-1 md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onCategoryFormChange("image", e.target.value)
                      }
                      className="w-full rounded-lg border border-border bg-background px-3 py-2"
                    />
                  </label>
